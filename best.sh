...
# Run this script to test database stability and connection pool
...
END_TOTAL=$(date +%s%N)
TOTAL_TEST_TIME=$(( ($END_TOTAL - $START_TOTAL) / 1000000000 ))  # Convert to seconds
...
if [ $SUCCESS_RATE -ge 99 ]; then
    ...
    echo "    1. Increase max_connections in config/default.yaml"
    echo "    2. Check PostgreSQL is running and responsive"
